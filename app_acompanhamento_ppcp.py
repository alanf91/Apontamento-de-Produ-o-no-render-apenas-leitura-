# VERSÃO 4.8: acrescenta consulta pública de paradas por setor, equipamento e data.
...
def carregar_paradas_cacheado(
    ...
    del modificacao_ns, tamanho_arquivo
    return carregar_paradas(Path(caminho_texto))


def obter_paradas_cacheadas(caminho: Path) -> pd.DataFrame:
    estatistica = caminho.stat()
    return carregar_paradas_cacheado(
        str(caminho.resolve()),
        estatistica.st_mtime_ns,
        estatistica.st_size,
    )
...
def renderizar_calendario_semanal(df: pd.DataFrame, caminho: Path) -> None:
    ...
    with aba_resumo:
        ...
        csv_resumo = resumo_exibicao.to_csv(index=False, sep=";", decimal=",", encoding="utf-8-sig")
        st.download_button(
...
def renderizar_consulta_paradas(caminho: Path) -> None:
    ...
    with aba_registros:
        ...
        csv = tabela.to_csv(index=False, sep=";", decimal=",", encoding="utf-8-sig")
        st.download_button(
...
def renderizar_apontamento_paradas(
    ...
    with st.form("form_apontamento_parada", clear_on_submit=True):
        ...
        criar_backup = st.checkbox("Criar backup antes de salvar", value=True)
        salvar = st.form_submit_button(
            "Salvar parada",
            type="primary",
            use_container_width=True,
    ...
    if paradas.empty:
        ...
        st.dataframe(
            exibicao[["Data Parada", "Setor", "Máquina/Posto", "Motivo da Parada", "Tempo Total Parada (min)", "Tempo de Parada", "Observação", "Registrado Em"]],
            use_container_width=True,
            hide_index=True,
            height=480,
        )
...
def renderizar_login_apontamento() -> None:
    ...
    st.caption(
        "As telas Produção por Setor, Calendário Semanal e Consulta de Paradas são públicas. "
        "Para lançar produção ou registrar paradas, informe o usuário e a senha."
    )
...
def renderizar_status_acesso() -> None:
    ...
    with st.sidebar:
        ...
        if apontamento_autenticado():
            ...
            st.caption("As consultas são públicas; os apontamentos de produção e paradas exigem login e senha.")
...
def main() -> None:
    ...
    if pagina_param in {"paradas", "parada", "apontamento-paradas"}:
        pagina_padrao = 4
    elif pagina_param in {"apontamento", "lancamento", "editar", "producao-apontamento"}:
        pagina_padrao = 3
    elif pagina_param in {"consulta-paradas", "paradas-leitura", "historico-paradas"}:
        pagina_padrao = 2
    ...
    pagina_protegida = "Apontamento" in pagina
    # As telas públicas sempre usam a planilha padrão do servidor.
    if not pagina_protegida:
    ...
    # A consulta pública de paradas depende apenas da aba 6_Paradas.
    ...
    if "Apontamento de Paradas" in pagina:
        renderizar_apontamento_paradas(df, caminho, nome_aba)
    elif "Apontamento de Produção" in pagina:
