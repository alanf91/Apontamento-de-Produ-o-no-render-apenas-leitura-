# VERSÃO 4.1: corrige a comparação programado x realizado usando as mesmas linhas da Data Programada.
...
def renderizar_login_apontamento() -> None:
    ...
    st.caption(
        "A consulta da Produção por Setor é pública. "
...
def main() -> None:
    ...
    pagina_padrao = 1 if pagina_param in {"apontamento", "lancamento", "editar"} else 0
    pagina = st.sidebar.radio(
        "Escolha a tela",
        ["📊 Produção por Setor — leitura", "🔐 Apontamento de Produção"],
        index=pagina_padrao,
    )
    st.sidebar.caption(
        "A Produção por Setor é aberta para consulta. "
        "O lançamento de apontamentos é protegido por login."
    )
    renderizar_status_acesso()

    # A tela pública sempre usa a planilha padrão do servidor.
    if "Produção por Setor" in pagina:
